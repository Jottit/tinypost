...
def test_draft_not_visible_on_public_site_page(client):
    ...
    with app.app_context():
        create_post(site["id"], "my-draft", "My Draft", "Draft body", is_draft=True)
...
def test_draft_not_in_rss_feed(client):
    ...
    with app.app_context():
        create_post(site["id"], "my-draft", "My Draft", "Draft body", is_draft=True)
        create_post(site["id"], "published", "Published", "Published body")
...
def test_toggle_draft_on_hides_from_public(client):
    user, site = _setup_site()
    with app.app_context():
        create_post(site["id"], "my-post", "My Post", "Post body")
...
def test_toggle_draft_off_shows_in_public(client):
    user, site = _setup_site()
    with app.app_context():
        create_post(site["id"], "my-post", "My Post", "Post body", is_draft=True)
